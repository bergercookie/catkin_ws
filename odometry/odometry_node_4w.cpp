...
{
	const long enc_res = 8515;
	float dt = float(data->looptime)/1000.;

	long encoderR = data->encoderR;
	long encoderL = data->encoderL;
	
	float dl = float(encoderR + encoderL)/float(enc_res)/2.*6.28*r;
	float dtheta = float(encoderR - encoderL)/float(enc_res)*6.28*r/2./d;

//	float lim=0.05;
	// olisthisi
//	if (abs(velR) < lim && abs(velL) >= lim)
//	{
//		dl = d*sin(dtheta); 
//	}
//	else if (abs(velR) >= lim && abs(velL) < lim)
//	{
//		dl = d*sin(dtheta);
//	}
	...
	velR = encoderR/enc_res/dt*2.*3.14*r;
	velL = encoderL/enc_res/dt*2.*3.14*r;	
...
int main(int argc, char** argv)
{
	...
	sub = nh.subscribe<arduino_feedback::feedback_int>("feedback", 5, &Callback);
